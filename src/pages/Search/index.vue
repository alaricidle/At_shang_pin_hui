      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-if="searchParams.trademark">{{searchParams.trademark.split(":")[1]}}<i @click="reMoveMark">×</i></li>
            <li class="with-x" v-for="(item,index) in searchParams.props">{{item.split(":")[1]}}<i @click="reMoveProps(index)">×</i></li>
          </ul>
        </div>

        <!--selector-->
        <SearchSelector @tradeInfo="tradeInfo" @arrt="arrt" />
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont" :class="{'icon-xiangxia4':isAsc,'icon-xiangshang4':isDesc}"></span></a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a>价格<span v-show="isTwo" class="iconfont" :class="{'icon-xiangxia4':isAsc,'icon-xiangshang4':isDesc}"></span></a>
                </li>
...
// this.searchParams.category2Id=this.$route.query.categor2Id
...
Object.assign(this.searchParams,this.$route.params,this.$route.query)
  ...
  //  },
   ...
   methods:{
    requestData(){
      this.$store.dispatch("search/reqSearchs",this.searchParams)
    },
    //移去路由中的名字面包屑
    reMoveCateName(){
    ...
    changeOrder(flag){
      ...
      let newOrder=`${flag}:${originSort=="desc"?"asc":"desc"}`
        ...
        let newOrder=`${originFlag=='1'?"2":"1"}:${originSort}`
        this.searchParams.order=newOrder
        this.requestData()
      }
    },
//自定义事件 得到分页器点击的数据，并请求数据
    getPageNo(data){
