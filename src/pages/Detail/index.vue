          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item,index1) in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" :class="{active:items.isChecked=='1'}"
                 v-for="(items,index2) in item.spuSaleAttrValueList" :key="items.id" @click="changeActive(items,item.spuSaleAttrValueList)">
                 {{items.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.$store.dispatch("detail/reqDetails",this.$route.params.skuid)
    },
    computed:{
 ...
 async addCart(){
      try{
         let result=await this.$store.dispatch("detail/addCartOrUpdataCart",{
          skuid:this.$route.params.skuid,
          skuNum:this.skuNum
        })
         //这里需要进行传参，可是使用路由传参的方式给下一级路由、
         //但是这里没有用，使用的是本地存储 但由于本地存储只能使用 字符串进行存储，所以需要将索要存储的对象，转化成字符串
         // 这里使用 会话存储 不进行持久化，会话结束数据就会消失
      sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))
      this.$router.push({name:"AddCartSuccess",query:{skuNum:this.skuNum,skuid:this.skuid}})
      }catch(error){
        console.warn(error)
      }
