  ...
  export default {
    ...
    methods:{
     async orderGet(){
        ...
        let res= await this.$api.payment(orderId)
        if(res.code==200){
          this.data=res.data
        }
     },
     async open(){
        ...
        this.$alert(`<img src="${url}" />`, '微信支付', {
              ...
              beforeClose:(action, instance, done)=>{
                if(action=='cancel'){
                  ...
                  // if(this.code==200){
                    ...
                    clearInterval(this.time)
                    this.time=null
                    this.$router.push("/paysuccess?orderId="+this.$route.query.orderId)
                    done()
                  }else{
                    alert("支付失败，请重试")
                  }
            ...
            let res=  await this.$api.queryPayStatus(this.$route.query.orderId)
            this.code=res.code
            if(res.code==200){
             clearInterval(this.time)
             this.timer=null
